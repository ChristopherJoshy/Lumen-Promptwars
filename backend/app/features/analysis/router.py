...
async def ask_about_case(case_id: str, body: dict) -> dict:
    ...
    try:
        answer = await service.answer_question(case_id, str((body or {}).get("question", "")))
    except ValueError as exc:
        message = str(exc)
        if message.startswith("Answer failed") or "unparsable" in message:
            raise HTTPException(status_code=502, detail=message) from exc
        status = 404 if "Unknown or expired" in message else 422
